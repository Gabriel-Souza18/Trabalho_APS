...
class TelaAluno():
    def show_tela_inicial_aluno(self, aluno, turma):
        sg.theme('DarkBlue12')

        layout = [
            [sg.Text('Nome: ' + aluno.nome, font=("Arial 14")), sg.Text('', size=(10, 1)),
             sg.Text(f'Matricula: ' + aluno.matricula, font="Arial 14")],
        ...
        while True:
            ...
            if event == "NOTAS":
                self.tela_notas(aluno, turma)

            if event == "TURMA":
                self.tela_turma(turma)
        window.close()

    def tela_notas(self, aluno, turma):
            ...
            if event == sg.WINDOW_CLOSED or event == 'FECHAR':
                ...
                if values['Tabela']:
                    ...
                    if materia is not None:
                        self.tela_materia(materia)

        window.close()

    def tela_turma(self, turma):
        layout = [
            [sg.Table(values=[],
